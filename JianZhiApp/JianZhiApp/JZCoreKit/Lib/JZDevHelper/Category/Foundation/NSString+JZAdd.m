...
- (CGFloat)jz_widthForFont:(UIFont *)font {
    CGSize size = [self jz_sizeForFont:font size:CGSizeMake(HUGE, HUGE) mode:NSLineBreakByWordWrapping];
    return size.width;
}


- (CGFloat)jz_heightForFont:(UIFont *)font width:(CGFloat)width {
    CGSize size = [self jz_sizeForFont:font size:CGSizeMake(width, HUGE) mode:NSLineBreakByWordWrapping];
    return size.height;
}
...
- (CGFloat)jz_heightForFont:(UIFont*)font maxLine:(NSInteger)maxLine width:(CGFloat)width {
    ...
    CGFloat rows = size.height / oneLineHeight;
    if (maxLine > 0 && rows > maxLine) {
        return (maxLine * oneLineHeight);
    }else {
        return size.height;
    }
}
...
- (BOOL)jz_exist
{
    return [[NSFileManager defaultManager] fileExistsAtPath:self isDirectory:nil];
}
...
{
    return [[NSFileManager defaultManager] copyItemAtPath:self
                                                   toPath:self
                                                    error:nil];
}
...
{
    return [[NSFileManager defaultManager] moveItemAtPath:self
                                                   toPath:self
                                                    error:nil];
}
...
- (BOOL)jz_remove
{
    return [[NSFileManager defaultManager] removeItemAtPath:self
                                                      error:nil];
}
...
- (NSDictionary *)jz_getFileInfoAtPath{
    return [[NSFileManager defaultManager] attributesOfItemAtPath:self error:nil];
}
